...
class NystromApproximation(TransformerMixin, BaseEstimator):
    ...
    """
    def __init__(self, n_components=100, kernel="rbf", random_state=None, **kernel_args):
        self.n_components = n_components
        self.random_state = random_state
        self.kernel_args = kernel_args  # Store kernel arguments for later use

        # Instantiate the kernel
        if isinstance(kernel, str):
            if kernel == "rbf":
                self.kernel = GaussianKernel(**kernel_args)
            elif kernel == "laplace":
                self.kernel = LaplacianKernel(**kernel_args)
            elif kernel == "linear":
                self.kernel = LinearKernel()
            elif kernel == "polynomial":
                self.kernel = PolynomialKernel(**kernel_args)
            elif kernel == "cauchy":
                self.kernel = CauchyLorentzKernel(**kernel_args)
            else:
                raise ValueError(f"Unsupported kernel: {kernel}.")
        elif isinstance(kernel, Kernel):
            self.kernel = kernel
        else:
            raise ValueError("`kernel` must be a string or an instance of the `Kernel` class.")
    ...
    def _get_kernel_function(self):
        """
        Returns the kernel function based on the kernel type or instance.
        """
        if not isinstance(self.kernel, Kernel):
            raise ValueError("`kernel` must be a valid Kernel instance.")
        return self.kernel
